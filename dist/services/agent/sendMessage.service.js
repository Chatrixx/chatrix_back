...
// Helper Functions -------------------
async function sendSummaryIfTriggered(input, clinic_id, client, contact_channel, messages) {
    const phoneNumber = extractTurkishPhoneNumber(input);
    if (!phoneNumber || !(messages.length > 0))
        return;
    const formatted = messages.map((msg) => ({
        sender: msg.role === MESSAGE_SENDER_TYPES.CHATRIX_AUTO_MESSAGE
            ? "assistant"
            : "client",
        text: msg.content,
    }));
...
async function getAggregatedReply(clientKey, assistantId) {
    if (pendingResponses.has(clientKey)) {
        ...
        const answer = await getOpenAiReply(mergedMessages, threadId, assistantId, openaiClient);
...
export default async function sendMessage(body, clinic_id) {
    ...
    if (!client) {
        const clientFullName = body.full_name ??
    ...
    const uniqueClientKey = `${clinic_id}_${client.channels[contact_channel]?.profile_info?.[channelPrimaryKey]}`;
    ...
    const handleSendReply = async () => {
        const answer = (await getAggregatedReply(uniqueClientKey, clinic_assistant_id));
        if (!answer) {
        ...
        await Client.updateOne({ _id: client._id }, {
            $set: {
                ...
                ...(phoneFromMessage && {
                    ...
                    [`channels.${contact_channel}.profile_info.phone`]: phoneFromMessage,
