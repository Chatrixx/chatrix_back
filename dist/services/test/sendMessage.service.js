...
// ------------------------------------------------------------
export default async function sendMessage(input, clinic_id) {
    if (!input) {
    ...
    const handleSendReply = async () => {
        ...
        await TestClient.updateOne({ _id: testUser._id }, {
            $set: {
    ...
    if (pendingResponses.has(uniqueUserKey)) {
        ...
        pending.timeout = setTimeout(handleSendReply, MAX_MESSAGE_INTERARRIVAL_DURATION);
    }
    else {
        const messages = [input];
        const timeout = setTimeout(handleSendReply, MAX_MESSAGE_INTERARRIVAL_DURATION);
