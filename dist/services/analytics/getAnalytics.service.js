...
const getAnalyticsFromDB = async (clinic_id, startDate, endDate, channel = CHANNELS.INSTAGRAM, groupBy = "day" // 'day' or 'month'
) => {
...
export default async function getAnalytics({ clinic_id, startDate, endDate, channel = CHANNELS.INSTAGRAM, groupBy = "day", }) {
    if (!clinic_id) {
        throw new ApiError(400, "clinic_id is required");
    }
    const analytics = await getAnalyticsFromDB(clinic_id, startDate, endDate, channel, groupBy);
    const total_messengers = analytics.reduce((acc, curr) => acc + curr.totalMessengers, 0);
    const total_phone_numbers_given = analytics.reduce((acc, curr) => acc + curr.totalPhoneNumbersGiven, 0);
    return {
        data_series: analytics,
        total_messengers,
        total_phone_numbers_given,
        total_phone_ratio: (total_phone_numbers_given ?? 0 / total_messengers).toFixed(2) * 1,
    };
}
